...
function build_for_arch() {
  ...
  ./configure --disable-static --without-zlib --enable-shared --enable-ipv6 ${SSL_FLAG} --host="${HOST}" --prefix=${PREFIX} && make -j8 && make install
}
...
${DEVROOT}/usr/bin/lipo \
  -arch i386 ${TMP_DIR}/i386/lib/libcurl.dylib \
  -arch x86_64 ${TMP_DIR}/x86_64/lib/libcurl.dylib \
  -arch armv7 ${TMP_DIR}/armv7/lib/libcurl.dylib \
  -arch armv7s ${TMP_DIR}/armv7s/lib/libcurl.dylib \
  -arch arm64 ${TMP_DIR}/arm64/lib/libcurl.dylib \
  -output ${TMP_DIR}/lib/libcurl.dylib -create
